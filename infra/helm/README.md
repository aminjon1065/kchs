...
2. **Файл значений вне репозитория**: `secrets.values.*`. Чарт создаёт `Secret`
   ...
   данным. Установка с тем же именем релиза в том же пространстве имён забирает
   оставшийся секрет обратно; установка под другим именем на него наткнётся —
   тогда либо `secrets.existingSecret=<оставшийся>`, либо удалить его руками,
   понимая, что зашифрованные им данные станут нечитаемыми.
