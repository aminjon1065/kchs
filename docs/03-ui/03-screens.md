...
Центрированная карточка на `--bg-canvas`: логотип/название организации (брендирование), логин/e-mail, пароль, «Войти», ссылка «Не удаётся войти», кнопка SSO (если включено). Второй шаг — MFA (6 цифр, автопереход, «использовать код восстановления»). Ошибки — без раскрытия существования учётной записи. После входа — «Мой день» (или «Продолжить»).
...
Реализовано (ADR-0095): экран «Знания» — слева дерево страниц пространства с поиском по названию и кнопкой «Новая страница» (шаблон выбирается в диалоге), справа — поиск по тексту страниц кусками с подсветкой и отметкой, участвовала ли смысловая выдача. Карточка страницы — вкладки «Страница» (блоки совместного документа, оглавление справа), «Версии» (сравнение с текущим текстом и откат) и «Пересмотр» (владелец, срок, ознакомление); в шапке — состояние, срок пересмотра, «Печать» и «Опубликовать». Кнопка блока «Обсудить фрагмент» открывает контекстную панель на комментариях к этому блоку. Режим «Спросить базу знаний» — вместе с семантическим поиском, отдельной работой.
