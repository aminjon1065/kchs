...
| `wait` | ждать событие (`event`, `filter`) или срок (`until`, `durationWorkingDays` или `durationHours`) | — |
...
- Сроки — в рабочих днях по бизнес-календарю от активации шага (`dueWorkingDays`) или в календарных часах (`dueHours`, ADR-0131); таймеры — отложенные задания.
...
**Сроки (ADR-0131).** У `approval`, `sign`, `register`, `acknowledge`, `task` и `return` срок задаётся одним способом: `dueWorkingDays` (0–365) — конец N-го рабочего дня по производственному календарю от активации шага, или `dueHours` (1–720) — ровно через N **календарных** часов от активации, без выходных и праздников (экстренные маршруты). У `wait` так же — `durationWorkingDays` или `durationHours`. Оба способа сразу — ошибка проверки `due_conflict`. Напоминания дневного срока — в 09:00 за рабочий день до срока и в день срока; часового — одно, за час до срока, а у срока короче двух часов — посередине; просрочка — в момент срока в обоих случаях.
...
**События** сверх перечисленных: `process.step_decided` (каждое решение; актор — кто нажал, `userId` — чья очередь), `process.step_assignees_changed` (добавлен согласующий, шаг передан или переназначен), `process.step_due_soon` (`when`: `before` — за рабочий день, `due_day` — в день срока, `soon` — незадолго до часового срока), `process.definition_changed` (черновик сохранён или снят, версия опубликована).
