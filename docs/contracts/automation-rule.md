...
Адресаты ограничены белым списком политики безопасности (ADR-0141): `send_email` пишет сотрудникам
из справочника и на явные адреса доменов `ruleEmailDomains`, `webhook` вызывает только домены
`ruleWebhookDomains` (и их поддомены). Адрес мимо списка — ошибка проверки при сохранении и
включении правила, а при запуске — отказ действия с той же причиной.
