...
```json
{
  ...
  "otherwise": [
    {"type": "add_tag", "tag": "обычный", "object": "{{object.id}}"}
  ],
...
`run_pipeline` (`pipelineId`) ставит прогон пайплайна, `run_import` (`sourceId`) — синхронизацию
источника данных (внешняя база, лента по адресу) — ADR-0163. Идентификатор — литерал или шаблон
`{{…}}`; нужно право служебного пользователя «запускать» пайплайн или «синхронизировать»
источник. Правило не ждёт конца прогона. Действия `update_fields` и
`set_status` работают для типов с поставщиком данных и переходами (документ, задача);
`assign` — для задач.

## Ветка «иначе»
`otherwise` — действия, когда условие не выполнено (по умолчанию `[]`, ADR-0163). Пустая ветка —
запуск «Пропущено», как раньше; без условий ветка не выполняется. Ветка запуска запоминается и
после `wait`; шаги журнала несут `branch: "otherwise"`; ключ повтора у ветки свой.

## Версии и перенос одного правила
Каждая правка определения — версия (`rule_versions`); откат делает старое определение новой
версией, состояние «включено» не меняется. Файл одного правила — `{"format": "kchs.rule",
"version": 1, "key", "exportedAt", "definition"}`: служебный пользователь снят, правило
выключено, секрет подписи вебхука и заголовки с ключами пусты. Импорт создаёт правило
выключенным; занятый ключ заменяется новым.
...
- Тестовый режим: прогон без действий («что бы произошло»): событие — на N последних событиях,
  входящий вызов — на последних вызовах с ключом правила, расписание и показатель — один запуск
  «сейчас», кнопка у объекта — на последних изменённых объектах нужных типов (ADR-0163).
