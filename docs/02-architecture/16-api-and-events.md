...
| data | `source.checked`, `dataset.created`, `dataset.schema_changed`, `dataset.import_started`, `dataset.imported`, `dataset.import_failed`, `dataset.rows_changed` (ids, op), `dataset.version_created`, `dataset.policies_changed` (kind, op), `chart.updated`, `dashboard.updated`, `query.executed`, `metric.evaluated`, `dashboard.published`, `report.generated`, `form.assigned`, `form.submitted`, `form.reviewed`, `alert.fired`, `quality.evaluated`, `pipeline.run_finished` |
