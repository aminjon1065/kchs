...
| tasks | `task.created`, `task.assigned`, `task.accepted`, `task.status_changed`, `task.due_changed`, `task.territory_changed` (from, to — ADR-0077), `task.reported`, `task.completed`, `task.returned`, `task.extension_requested`, `task.overdue`, `task.due_soon`, `project.created` |
...
- `documents.public.createFromReport({reportRunId, typeId, subject, fileId})`, `documents.public.registerProtocol({protocolId, typeId})`. Реализовано (ADR-0080): `DocumentsPublic.applyTransition(tx, ctx, documentId, {to, cause, source?})` — единственная смена статуса по графу жизненного цикла; `DocumentsPublic.setParticipants(tx, ctx, documentId, source, entries)` — права участников одного источника (`route:<экземпляр>`, `resolution:<id>`); `DocumentsPublic.register(tx, ctx, documentId, {journalId?, reservationId?})`; `DocumentsPublic.brief(executor, documentId)` — реквизиты после проверки `view` вызывающим.
