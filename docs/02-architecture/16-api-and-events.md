...
| gis | `layer.published` (datasetId, geometryType), `layer.style_changed` (changed), `map.updated` (changed), `feature.created`, `feature.updated`, `feature.deleted`, `feature.edit_submitted`, `feature.edit_reviewed`, `analysis.created`, `analysis.queued`, `analysis.started`, `analysis.finished` (status, datasetId, rows), `territory.updated` |
