...
- Реализовано (ADR-0113): ящик канцелярии — интеграция `imap` (конфигурация, зашифрованный пароль, «Проверить соединение», журнал синхронизаций); опрос — в общем планировщике. Письмо становится черновиком входящего: тема, текст письма в «суть», корреспондент по адресу, а не нашёлся — по почтовому домену ведомства из справочника корреспондентов (белый список, самый точный домен, общие почтовые сервисы не сопоставляются; ADR-0136), иначе очередь предлагает завести, вложения — файлами документа, а первое из них — основным файлом первой версии, то есть сканом. Объектом реестра письмо не становится: им становится черновик, а письмо остаётся записью очереди «Из почты» (способность `documents.register`) с предпросмотром, кнопкой «Зарегистрировать» (обычная карточка и номер из журнала) и отклонением с причиной. Дедупликация — уникальный ключ (ящик, `Message-ID`); письмо, не прошедшее правила отбора или не разобравшееся, помечается в очереди с причиной, а не теряется. Отклонённые и неразобранные записи очереди удаляются через 180 дней после решения или получения — задание обслуживания `documents.mail-purge` единого планировщика раз в сутки, событие `mail.purged` (ADR-0136). Ответы письмом в обсуждение документа не принимаются: адрес отправителя слабо удостоверяет личность (ADR-0136).
...
- **Переписка**: «Ответить» на зарегистрированный входящий — черновик исходящего с темой, корреспондентом и грифом входящего, связь `reply_to`; отметки об отправке (реестр отправки) — первая исполняет исходящий и готовит исполнителю поручения по резолюции входящего отчёт с номером исходящего (подписчик `documents-reply-report`: исполнитель — автор или ответственный исходящего, иначе единственное открытое основное поручение; ADR-0136); цепочка переписки по `reply_to` в обе стороны, недоступные документы — без реквизитов.
...
- **Номенклатура из Excel и типовая номенклатура** (ADR-0135): «Импорт из Excel» в справочнике номенклатуры — образец с типовой номенклатурой, проверка строк без изменений, создание готовых дел одной транзакцией (`POST /cases/import`, `GET /cases/import/template.xlsx`); книга Excel читается в api без движка (`shared/xlsx.ts`). Демо-сид заводит типовую номенклатуру по подразделениям на прошлый и текущий год.
