import type { Bbox, LayerRecord, MapCamera, MapSpec } from '@kchs/contracts'
import {
  AlertDialog,
  Button,
  EmptyState,
...
export function MapStudio({
  ...
  const onFeatureClick = (event: MapClickEvent) => {
    ...
    setPicked({ layerId, rowId: String(hit.id), point: event.point })
  }
  ...
  return (
    <div ref={setRoot} className="flex h-full min-h-0 flex-col">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 md:grid-cols-[280px_minmax(0,1fr)]">
        <aside className="hidden min-h-0 flex-col border-r border-line bg-surface-2 md:flex">
          <LayerPanel
            layers={panelLayers}
