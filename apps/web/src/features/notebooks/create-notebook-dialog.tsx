 ...
 * текстовой ячейкой — можно сразу писать; открывается во вкладке.
 */
export function CreateNotebookDialog({
  ...
  return (
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent
        title={t('data.notebook.createTitle')}
        size="sm"
        footer={
          <>
      ...
      >
        <div className="flex flex-col gap-3">
          ...
          <Field label={t('data.notebook.name')}>
            <Input
              autoFocus
              value={name}
              ...
              aria-label={t('data.notebook.name')}
            />
          </Field>
        </div>
      </DialogContent>
